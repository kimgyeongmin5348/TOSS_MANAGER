...
URL : https://tossmanager-knkphcrq2pumbwxrj8c2qg.streamlit.app/
...
- 개인화된 환영 화면, 컬러 자산 카드, 집중도 링으로 핵심 상태를 시각화
- 실시간 연결 직후와 제한된 장중 간격으로 최신 포트폴리오 저장
- 사용자가 필요할 때 직접 저장할 수 있으며 동일한 보유 상태는 해시로 중복 방지
- 한국·미국 시장의 장 마감 스냅샷은 장중 기록과 분리하여 장기 보존
- 일반 장중 기록은 30일 보관 정책으로 무제한 증가 방지
- 다른 PC에서 로그인해도 마지막 저장 포트폴리오, 보유 종목, 평균 단가, 저장 차트 조회
- 저장 데이터 모드에서 데이터 기준 시각·DB 저장 시각·저장 상태를 명확히 표시
- DB 저장이 실패해도 Toss 실시간 계좌 조회는 중단하지 않고 별도로 재시도
...
- 종목 상세 화면에서 관심종목 추가·해제
- 사용자별 메모와 목표가격을 TiDB에 저장하여 재로그인 후에도 유지
- 실시간 현재가와 목표가격의 차이 및 목표 도달 여부 표시
- 국내·미국 시장 필터와 최근 뉴스 건수·심리·최신 제목 요약
- 모든 조회와 수정 조건에 `user_id`를 포함해 사용자 데이터 완전 분리

### 8. 로그인 보안과 계정 관리

- 로그인 5회 실패 시 15분 임시 잠금으로 반복 공격 제한
- 30분 유휴 시간과 8시간 절대 시간을 적용한 세션 만료
- 비밀번호 변경 시 기존 로그인 세션을 무효화
- 연결된 토스 계좌 확인·해제, 통합 로그아웃, 회원탈퇴 지원
- 탈퇴 시 사용자·계좌·스냅샷·관심종목은 삭제하고 비식별 시장 데이터만 보존
- 사용자명·종목명·뉴스 등 HTML 동적 값을 이스케이프하여 삽입 공격 차단
